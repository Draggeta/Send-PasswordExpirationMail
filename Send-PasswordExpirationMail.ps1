...
# Search all users whom have enabled accounts and have expiring passwords
Get-ADUser -Filter $ADFilter -SearchBase $SearchBase -SearchScope Subtree -Properties PasswordLastSet,EmailAddress | Where-Object $WhereFilter | ForEach-Object {
    ...
    If ($IntervalHit) {
        ...
        $ExpirationDays = If ($DaysLeft -eq 0) {'today'}
        ElseIf ($DaysLeft -eq 1) {'in one day'}
        Else {"in $DaysLeft days"}

        # Automatically apply or omit the text about complexity dependant on group policy
        $Complexity = If ($ComplexityEnabled) {'<li>Het moet tekens uit minimaal 3 van de 4 onderstaande categorieën bevatten:</li>
        <ul><li>Minstens een <em>kleine letter</em> (a-z)</li>
        <li>Minstens een <em>hoofdletter</em> (A-Z)</li>
        <li>Minstens een <em>cijfer</em> (0-9)</li>
        <li>Minstens een <em>speciale teken</em> (!,?,*,~, etc.)</li></ul>'}



        $Message = "<p>Dear $Name,<br></p>
        <p>Your password will expire <em>$ExpirationDays</em>. You can change your password by pressing CTRL+ALT+DEL and then choosing the option to reset your password. If you are not at the Leiden or WTC campus, you can reset your password via the webmail. Click on the gear icon and subsequently on `"Change password`" to change your password.</p>
        <p>If you have set up your Webster.nl email account on other devices such as an iPhone/iPad or an Android device, please change your password on those devices as well.</p>
        ...
        Send-MailMessage -To $Email -From $From -Subject "Reminder: Your password expires $ExpirationDays" -Body $Message -SmtpServer $SMTPserver -BodyAsHTML -Credential $Credential -UseSsl -Port 587
...
# Created by Tony Fortes Ramos on 2014-04-24
# Last edited by Tony Fortes Ramos on 2015-08-02